...
/**
 * Transformer: wknd-trendsetters.site section boundaries + section metadata.
 *
 * Inserts a section break (<hr>) before each non-first section and a
 * "Section Metadata" block for each section that carries a distinct style.
 *
 ...
 * section boundaries and styles from migration-work/authoring-analysis.json are
 * embedded below. If a future page-templates.json populates
 * `payload.template.sections`, that takes precedence.
 *
 * Selectors verified against migration-work/cleaned.html (main#main-content
 * children: 1 header + 6 sections). Styles per authoring-analysis
 * step3eValidation:
 *  - rc1 header.secondary-section ......... style: secondary (KEEP)
 *  - rc2 section:nth-of-type(1) ........... default white (no metadata)
 *  - rc3 section.secondary:nth-of-type(2) . style: secondary (KEEP)
 *  - rc4 section:nth-of-type(3) ........... default white (no metadata)
 *  - rc5 section.secondary:nth-of-type(4) . style: secondary (KEEP)
 *  - rc6 section:nth-of-type(5) ........... default white (no metadata)
 *  - rc7 section.inverse-section .......... SKIP metadata (bg image is the
 *        hero-overlay block's own design, not section styling)
 */
...
const EMBEDDED_SECTIONS = [
  ...
  { id: 'rc7', selector: '#main-content > section.section.inverse-section' },
];
